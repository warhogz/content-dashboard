      ...
      <Card>
        <CardContent className="space-y-4 p-5">
          ...
          <form key={selected?.id || "new"} onSubmit={submit} className="grid gap-3 md:grid-cols-[1fr_1fr_120px_1fr_auto]">
            ...
            <Input name="color" type="color" defaultValue={selected?.color || "#f43f5e"} className="h-11 w-full rounded-2xl border border-white/10 bg-white/5 p-1" />
            <div className="flex flex-wrap gap-3">
              ...
              <label className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 text-sm text-white/80"><Checkbox name="show_on_public" defaultChecked={selected?.show_on_public ?? true} /> Публичный</label>
            </div>
            <Button type="submit" disabled={pending}>{selected ? "Обновить" : "Добавить"}</Button>
          </form>
        </CardContent>
      </Card>
          ...
          <Card key={status.id}>
            <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <div className="text-sm text-white/45">{status.slug} · #{status.color.replace("#", "")}</div>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setSelected(status)}>Редактировать</Button>
                <Button variant="ghost" onClick={() => del(status.id)}>Удалить</Button>
              </div>
            </CardContent>
          </Card>
        ))}
